...
SRC_DIRS := assemblers devices inputs mounts osbuild runners sources stages tools
...
SRC := $(shell find . -name "*.py") \
       $(foreach dir,$(SRC_DIRS),$(call find_python_files,$(dir)))
...
$(RPM_DEVEL): $(RPM_SPECFILE_DEVEL) $(RPM_TARBALL_DEVEL) $(SRC)
